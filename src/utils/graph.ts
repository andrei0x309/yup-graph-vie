...
let width, height, svg, tooltip, simulation, link, node;
const cacheDuration = 1800000; // 1800000 = 30min cache
const threeDimensional = true;
...
function addSelectNode(node, myForceGraph) {
...
// eslint-disable-next-line no-unused-vars
function clearSelectedNodes(myForceGraph) {
...
export async function getSingleUserData(user, limit=1000){
    let fullData = [];
...
async function getUserData(users, limit=1000) {
  let fullData = [];
  await Promise.all(
...
async function filter() {
  ...
  if (threeDimensional) {
    setSpinner('3D');
    draw3D(filteredData, myForceGraph);
  } else {
    setSpinner('normal');
    draw(filteredData);
  }
...
function draw3D(data, myForceGraph) {
  updateDetailsTab();
